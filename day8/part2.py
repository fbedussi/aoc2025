...
def main(isTest):
    ...
    while True:
        [a, b] =list(map(int, sorted_distances[i][0].split('_')))
        group(a, b)
        i += 1
        if len(set(map(lambda x: x[1], data))) == 1 and data[0][1] != None:
            start = a
            end = b
            break

    return data[start][0][0] * data[end][0][0]


class Test(unittest.TestCase):
    def test_test_data(self):
        self.assertEqual(main(True), 25272)

    def test_real_data(self):
        self.assertEqual(main(False), 9617397716)
